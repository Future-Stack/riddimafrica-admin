...
export function ModalShell({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
      <div
        className={`bg-white ${roundedClassName} w-full ${maxWidthClassName} p-6 font-inter shadow-xl max-h-[90vh] overflow-y-auto`}
      >
        <div className="flex items-start justify-between mb-6">
          {header ?? (
            <div>
              <h2 className="text-xl md:text-2xl font-bold text-[#3E2723] leading-7 mb-1.5">
                {title}
              </h2>
              {subtitle && (
