...
export const StatsCardHeader: React.FC<StatsCardHeaderProps> = ({
  ...
  return (
    <div className="flex items-center justify-between w-full mb-4">
      ...
      <div
        className={`p-2 rounded-lg text-white flex items-center justify-center ${iconBgColor}`}
      >
        {icon}
      </div>
    </div>
...
export const StatsCardFooter: React.FC<StatsCardFooterProps> = ({
  ...
  const baseButtonStyles =
    "flex items-center gap-1 font-medium text-[#E6A400] hover:text-amber-300 font-inter transition-colors group text-sm leading-6";
...
export const StatsCard: React.FC<StatsCardProps> = ({
  ...
  return (
    <div
      className={`flex flex-col p-5 rounded-xl text-white shadow-lg min-w-[260px] flex-1 ${bgColor}`}
    >
      <StatsCardHeader {...headerProps} />

      <div className="mt-2">
        <h2 className="text-2xl font-bold  tracking-tight md:text-3xl text-[#E6E8EB]">
          {value}
        </h2>
      </div>
