...
export default function NotificationModal({
    ...
    return (
        ...
        >
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b border-[#DFE3E880]">
                ...
                <div className="flex items-center gap-3">
                    {unreadCount > 0 && (
                        <button
                            onClick={markAllRead}
                            className="flex items-center gap-1 text-xs text-[#c19a56] font-medium hover:text-[#b08a48] transition-colors"
                        >
                            <CheckCheck size={14} />
                            Mark all
                        </button>
            ...
            {/* Body */}
            <ul className="max-h-[360px] overflow-y-auto divide-y divide-[#DFE3E880]">
                {notifications.length === 0 ? (
                    ...
                    notifications.map((item) => (
                        <li
                            ...
                            className={`flex gap-3 px-5 py-4 cursor-pointer hover:bg-[#FAF7F3] transition-colors ${!item.read ? "bg-[#f3ede4]/40" : ""
                                }`}
                        >
                            <span
                                className={`w-2 h-2 rounded-full mt-2 flex-shrink-0 ${item.read ? "bg-gray-300" : typeStyles[item.type]
                                    }`}
                            />

                            <div className="flex-1 min-w-0">
                                <p className="font-semibold text-sm text-[#0a192f]">{item.title}</p>
                            ...
                            <div className="flex gap-1 flex-shrink-0">
                                {!item.read && (
                                    <button
                                        ...
                                        className="text-gray-400 hover:text-[#166534] transition-colors p-1"
