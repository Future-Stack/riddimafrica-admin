...
import { AnalyticsIcon, ArtistsIcon, DashboardIcon, LogoutIcon, OrderIcon, ProductsIcon, RadioIcon, RevenueIcon, SellersIcon, SettingsIcon, UsersIcon } from "./SidebarIcon";
...
export default function Sidebar({ sidebarOpen, setSidebarOpen }: SidebarProps) {
    ...
    return (
        <>
            ...
            <aside
                className={`
          bg-[#3E2413] text-white w-[260px] flex flex-col rounded-2xl
                            ...
                            return (
                                <li key={item.label}>
                                    <Link
                                        href={item.path}
                                        onClick={() => setSidebarOpen(false)}
                                        className={`flex items-center gap-4 px-4 py-3 text-base rounded-lg transition-all duration-200 group ${active
                                            ? "bg-[#63542C] text-white font-bold [#C9A96C]"
                                            : "text-white hover:bg-white/5 hover:text-white"
                                            }`}
                                    >
                                        <span className={active ? "text-white" : "text-white group-hover:text-white"}>
                                            {item.icon}
                                        </span>
                    ...
                    <ul className="space-y-2 mt-auto pb-6 pt-8">
                        ...
                        <li>
                            <Link
                                ...
                                onClick={() => setSidebarOpen(false)}
                                className={`flex items-center gap-4 px-4 py-3 rounded-lg transition-all duration-200 group ${pathname.startsWith("/dashboard/settings")
                        ...
                        <li>
                            ...
                            >
                                ...
                                <span className="text-sm tracking-wide text-red-500">Log Out</span>
                            </button>
                        </li>
                    </ul>
                </nav>
