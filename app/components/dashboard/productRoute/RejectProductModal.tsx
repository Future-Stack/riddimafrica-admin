...
export function RejectProductModal({ isOpen, productName, onClose, onConfirm }: RejectProductModalProps) {
            ...
            header={
                <div className="flex-1 pl-6">
                    <h2 className="text-lg sm:text-xl font-semibold text-[#101828] leading-7 text-center">
                        Are you sure you want to reject{productName ? ` "${productName}"` : " the product"}?
                    </h2>
                </div>
            }
        >
            <div>
                <label className="mb-1.5 block text-sm font-medium text-[#787A7F] leading-5">Reason</label>
                <textarea
                    value={feedback}
