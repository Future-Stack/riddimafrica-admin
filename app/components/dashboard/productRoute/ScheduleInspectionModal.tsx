    ...
    return (
        <div className="relative" ref={containerRef}>
            ...
            >
                ...
                <ChevronDown size={16} className={`text-[#377A7D] shrink-0 transition-transform ${open ? "rotate-180" : ""}`} />
            </button>
                ...
                <div className="absolute z-10 mt-1 w-full rounded-lg border border-[#5F9597] bg-white shadow-lg overflow-hidden">
                    {options.map((opt) => (
                        <button
                            ...
                            className={`w-full text-left px-3 py-2.5 text-sm cursor-pointer hover:bg-[#EBF2F2] ${opt.id === value ? "bg-[#EBF2F2] font-medium text-[#101828]" : "text-gray-700"
                                }`}
                        >
                            {opt.label}
                        </button>
...
export function ScheduleInspectionModal({
    ...
    const canConfirm = date.length > 0 && time.length > 0 && locationId.length > 0;
    ...
    return (
        ...
        >
            <div className="mb-4">
                <label className="mb-1.5 block text-sm font-medium text-[#787A7F] leading-5">Inspection Date</label>
                <div className="relative">
                    ...
                    />
                    <Calendar size={16} className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-[#377A7D]" />
                </div>
                {displayDate && (
            ...
            <div className="mb-4">
                <label className="mb-1.5 block text-sm font-medium text-[#787A7F] leading-5">Inspection Time</label>
                <div className="relative">
                    <input
                        type="time"
                        value={time}
                        ...
                        className="w-full rounded-lg border border-[#5F9597] bg-[#EBF2F2] px-3 py-2.5 pr-10 text-sm text-[#101828] focus:outline-none focus:ring-2 focus:ring-[#036B2C]/20"
                    />
                    <Clock size={16} className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-[#377A7D]" />
                </div>
            </div>

            <div className="mb-4">
                <label className="mb-1.5 block text-sm font-medium text-[#787A7F] leading-5">Inspection Location</label>
                <LocationDropdown value={locationId} onChange={setLocationId} options={locations} />
            </div>

            <div className="mb-5">
                <label className="mb-1.5 block text-sm font-medium text-[#787A7F] leading-5">Note to Seller (optional)</label>
                <textarea
                    value={note}
