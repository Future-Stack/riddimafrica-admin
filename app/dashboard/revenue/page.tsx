...
import { Clock, Percent, ShoppingCart, Wallet, Download } from "lucide-react";
import RevenueTrendChart, { RevenueTrendPoint } from "@/app/components/dashboard/revenueRoute/RevenueTrendChart";
import { RevenueSplitChart, RevenueSplitSlice } from "@/app/components/dashboard/revenueRoute/RevenueSpilt";
import { CategoryRevenueItem, CategoryRevenueList } from "@/app/components/dashboard/revenueRoute/ByCategory";
...
export type PayoutStatus = "Payout completed" | "Ready" | "Processing" | "Requested";
...
const INITIAL_PAYOUTS: PayoutRow[] = [
    { id: 1, sellerName: "AfroBeatsNG", sellerRole: "Seller", orderId: "ORD-0001", paymentMethod: "Airtel Money**4521", amountDueUGX: 1240000, status: "Payout completed" },
    { id: 2, sellerName: "AfroBeatsNG", sellerRole: "Artist", orderId: "ORD-0001", paymentMethod: "Airtel Money**4521", amountDueUGX: 1240000, status: "Ready" },
    { id: 3, sellerName: "AfroBeatsNG", sellerRole: "Seller", orderId: "ORD-0001", paymentMethod: "Airtel Money**4521", amountDueUGX: 1240000, status: "Processing" },
    { id: 4, sellerName: "AfroBeatsNG", sellerRole: "Seller", orderId: "ORD-0001", paymentMethod: "Airtel Money**4521", amountDueUGX: 1240000, status: "Processing" },
    { id: 5, sellerName: "AfroBeatsNG", sellerRole: "Seller", orderId: "ORD-0001", paymentMethod: "Airtel Money**4521", amountDueUGX: 1240000, status: "Processing" },
    { id: 6, sellerName: "AfroBeatsNG", sellerRole: "Seller", orderId: "ORD-0001", paymentMethod: "Airtel Money**4521", amountDueUGX: 1240000, status: "Processing" },
    { id: 7, sellerName: "Teni", sellerRole: "Artist", orderId: "ORD-0002", paymentMethod: "MTN MoMo**7732", amountDueUGX: 860000, status: "Requested" },
    { id: 8, sellerName: "Riddim Prints", sellerRole: "Seller", orderId: "ORD-0003", paymentMethod: "Airtel Money**9012", amountDueUGX: 430000, status: "Requested" },
];
...
export default function RevenueManagementPage() {
    ...
    const filteredPayouts = useMemo(
        () => payouts.filter((p) => (activeTab === "queue" ? p.status !== "Requested" : p.status === "Requested")),
        [payouts, activeTab]
    );
    ...
    const handleReleasePayout = (id: number) => {
        setPayouts((prev) => prev.map((p) => (p.id === id ? { ...p, status: "Payout completed" } : p)));
    };
    ...
    const columns: Column<PayoutRow>[] = [
        {
            ...
            render: (row) => (
                <div className="text-left font-inter">
                    <p className="text-sm text-gray-800 font-medium leading-6">{row.sellerName}</p>
                    <p className="text-xs text-[#787A7F] font-normal leading-4.5">{row.sellerRole}</p>
                </div>
            ),
        },
        ...
        { header: "Payment Method", key: "paymentMethod" },
        {
            header: "Amount Due",
            key: "amountDueUGX",
            render: (row) => <span>UGX {(row.amountDueUGX / 1_000_000).toFixed(2)}M</span>,
        },
        {
            header: "Status",
            key: "status",
        ...
        },
        {
            ...
            render: (row) => (
                ...
                >
                    Release Payout <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
    ...
    return (
        <div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-4 mb-5">
                <RevenueTrendChart data={REVENUE_TREND} range={trendRange} onRangeChange={setTrendRange} />
                    ...
                    <div className="flex flex-wrap items-center gap-2 mb-4">
                        {PAYOUT_TABS.map((tab) => (
                            <button
                                ...
                                className={`px-5 py-2 rounded-[6px] text-base font-medium font-inter leading-5 cursor-pointer transition-colors ${activeTab === tab.key
                        ...
                        <a href="#" className="ml-auto text-xs font-semibold text-[#E6A400] hover:opacity-80">
                            View all →
                        </a>
                    </div>
