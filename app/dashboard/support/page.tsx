...
import { SupportChatView } from "@/app/components/dashboard/supportRoute/SupportChatView";
import { SupportTicketsTable } from "@/app/components/dashboard/supportRoute/SupportTicketTable";
...
export default function SupportTicketsPage() {
    ...
    const handleSendMessage = (ticketId: string, messageText: string) => {
        ...
        setTickets((prev) =>
            prev.map((t) =>
                t.id === ticketId
                    ? { ...t, messages: [...t.messages, newMessage], lastMessage: messageText }
                    : t
            )
        );
        ...
        if (activeTicket && activeTicket.id === ticketId) {
            setActiveTicket((prev) =>
                prev
                    ? { ...prev, messages: [...prev.messages, newMessage], lastMessage: messageText }
                    : null
            );
        }
    };
