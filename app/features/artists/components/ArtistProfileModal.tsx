...
const StatChip: React.FC<{ label: string; value: string }> = ({
  label,
  value,
}) => (
  <div className="bg-[#F9F5EF] rounded-[8px] px-4 py-3 ">
    <p className="text-sm text-[#787A7F] font-medium font-inter leading-5.5 mb-1.5">
      {label}
    </p>
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-start justify-between gap-3 ">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-[#E6A400] font-medium font-inter leading-5">
                @{artist.stageName}
              </p>
              <div className="flex items-center gap-2 mt-1">
                <span className="text-xs font-normal font-inter px-3 py-1.5 rounded-full bg-[#F4D24233] text-[#E5B54F]">
                  {artist.genre}
                </span>
                <span className="text-xs font-normal font-inter px-3 py-1.5 rounded-full bg-[#F4D24233] text-[#E5B54F]">
        ...
        <div className="flex flex-col gap-3 ">
          <label className="flex items-center justify-end gap-2 cursor-pointer text-sm font-medium text-black leading-5 font-inter">
            ...
            >
              <div
                className={`absolute top-0.5 h-5 w-5 bg-white border border-gray-300 rounded-full transition-all ${isAuthorized ? "left-[22px]" : "left-[2px]"}`}
              />
            </div>
          </label>
          <label className="flex items-center justify-end gap-2 cursor-pointer text-sm font-medium text-black leading-5 font-inter">
            ...
            >
              <div
                className={`absolute top-0.5 h-5 w-5 bg-white border border-gray-300 rounded-full transition-all ${isFeatured ? "left-[22px]" : "left-[2px]"}`}
              />
            </div>
          </label>
        </div>
      ...
      <div className="mb-13">
        ...
        <div className="flex flex-wrap gap-2">
          {artist.merchandiseItems.map((item) => (
            <span
              key={item}
              className="text-sm sm:text-base font-medium font-inter leading-6 px-5 py-3 rounded-[8px] bg-[#EBF2F2]  text-green-600 "
            >
              {item}
            </span>
          ))}
