        ...
        >
            ...
            <div className="flex items-start gap-8 mb-8">
                <div className="shrink-0">
                    <p className="text-2xl md:text-3xl font-bold text-[#64284E] leading-8">{data.averageRating.toFixed(1)}</p>
                    ...
                    <p className="text-xs text-[#787A7F] font-bold leading-4 mt-1">{data.totalReviews} reviews</p>
                </div>
                ...
                <div className="flex-1 space-y-2">
                    {data.breakdown.map((b) => (
                        <div key={b.star} className="flex items-center gap-2">
                            ...
                            <span className="text-xs text-[#787A7F] font-normal w-4 text-right">{b.count}</span>
                        </div>
                    ))}
                </div>
            </div>
            ...
            <div className="space-y-5">
                {data.reviews.map((review) => (
                    <div key={review.id} className=" first:border-t-0 first:pt-0">
                            ...
                            <div className="flex items-center gap-2.5">
                                ...
                                <div>
                                    <p className="text-sm font-medium text-gray-800 leading-5">
                                        {review.name}<span className="text-xs font-normal text-gray-700 leading-4">@{review.handle}</span>
                                    </p>
                                    <div className="flex items-center gap-1.5 mt-1.5">
                                        ...
                                        <span className="text-xs text-[#787A7F] font-bold leading-4.5 ">{review.date}</span>
                                    </div>
                                </div>
                            </div>
                        ...
                        <p className="text-sm font-bold text-gray-800 leading-4.5 mt-2">{review.title}</p>
                        <p className="text-sm text-gray-800 mt-1 leading-5 font-normal">{review.body}</p>
                    </div>
                ))}
