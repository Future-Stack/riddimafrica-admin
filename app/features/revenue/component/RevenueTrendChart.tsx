...
function CustomTooltip({ active, payload, label }: any) {
    ...
    const revenue = payload.find((p: any) => p.dataKey === "revenueUGX")?.value ?? 0;
    const payout = payload.find((p: any) => p.dataKey === "payoutUGX")?.value ?? 0;
    return (
    ...
    return (
        <div className="bg-[#FAF7F3] rounded-xl border border-[#C4CDD566] font-inter p-5">
            <div className="flex items-start justify-between mb-4 flex-wrap gap-3">
                <div>
                    <h3 className="text-base md:text-lg font-medium text-[#101828] font-inter leading-7">Revenue Trend</h3>
                    <p className="text-xs text-[#624D3B] font-medium leading-4 mt-0.5">Monthly revenue vs payouts</p>
                </div>
