...
export function SupportTicketsTable({
  ...
  const columns: Column<Ticket>[] = [
    ...
    },
    {
      ...
      render: (row) => (
        <span
          className={`px-3 py-2 rounded-full font-normal text-xs inline-block ${getStatusBadge(row.status)}`}
        >
          {row.status}
        </span>
      ),
    },
    {
      ...
      render: (row) => (
        ...
        >
          ...
          >
            <path
              d="M3.78516 8.52346L6.30861 6.00001L3.78516 3.47656"
              stroke="#E6A400"
              stroke-width="0.84115"
  ...
  return (
    <div className="w-full  min-h-screen">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
          <PageHeader title="Support Tickets" description="" />
          <p className="text-xs sm:text-sm text-[#787A7F] font-normal mt-1">
            <span className="text-[#E6A400]">5 unread</span> • 4 open • 1
            pending • 2 resolved
          </p>
        </div>
